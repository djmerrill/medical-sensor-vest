...
char ser_buff[17]; // add 4 more blocks for six sensors in a row

// extreme values, or whatever they have other formal names, would be easier to process and have less impacts according to the empirical char histogram
char zero_char = 127;
char newline_char = 128;
...
void loop() {
  ...
  /*
   * I leave the correct acceleration values in this comment block.
   * We will take the LSB for computation.
  x = (int) word(values[1], values[0]); 
  y = (int) word(values[3], values[2]);
  z = (int) word(values[5], values[4]);
  */
  ser_buff[0] = (char) values[0];
  ser_buff[1] = (char) values[2];
  ser_buff[2] = (char) values[4];
  

  readRegister(DATAX0, 2, values, CS2);
  ser_buff[3] = (char) values[0];
  ser_buff[4] = (char) values[2];
  ser_buff[5] = (char) values[4];
  

  readRegister(DATAX0, 3, values, CS3);
  ser_buff[6] = (char) values[0];
  ser_buff[7] = (char) values[2];
  ser_buff[8] = (char) values[4];

  readRegister(DATAX0, 4, values, CS4);
  ser_buff[9] = (char) values[0];
  ser_buff[10] = (char) values[2];
  ser_buff[11] = (char) values[4];

  readRegister(DATAX0, 5, values, CS5);
  ser_buff[12] = (char) values[0];
  ser_buff[13] = (char) values[2];
  ser_buff[14] = (char) values[4];
  ser_buff[15] = newline_char;

  /*
  readRegister(DATAX0, 6, values, CS6);
  
  ser_buff[16] = (char) values[0];
  ser_buff[17] = (char) values[2];
  ser_buff[18] = (char) values[4];
  ...
  */
  ...
  ser_buff[16] = '\0'; // 20 for 6 sensors
